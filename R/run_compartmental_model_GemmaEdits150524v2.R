  ...
  #  Monthly_pop_growth <- 0 # remove
    ...
    for (month in 1:Num_timesteps) {
      ...
      if (species == "cattle" || species == "poultry" || species == "smallruminants") {
        ...
        res_vec$Offtake_SubAF[month] <- (1-(1-sample(Gamma_SubAF, 1))^(1/Num_timesteps)) * SubAF
        res_vec$Offtake_AF[month] <- (1-(1-sample(Gamma_AF, 1))^(1/Num_timesteps)) * AF
        res_vec$Offtake_SubAM[month] <- (1-(1-sample(Gamma_SubAM, 1))^(1/Num_timesteps)) * SubAM
      ...
      if (species == "cattle" || species == "poultry" || species == "small_ruminants") {
        res_vec$Growth_JF[month] <- (1/(sample(Beta_JF, 1))) * JF
        res_vec$Growth_SubAF[month] <- (1/(sample(Beta_SubAF, 1))) * SubAF
        res_vec$Growth_JM[month] <- (1/(sample(Beta_JM, 1))) * JM
        res_vec$Growth_SubAM[month] <- (1/(sample(Beta_SubAM, 1))) * SubAM
      } 
      ...
      if (species == "cattle" || species == "smallruminants" || species == "poultry") {
        ...
        res_vec$Culls_AF[month] <- (1/sample(CullF, 1)) * AF
        res_vec$Culls_AM[month] <- (1/sample(CullM, 1)) * AM
      }
...
## Deaths, Offtake and Culls equations are redone as above
        res_vec$Deaths_Ox[month] <- (1-(1-sample(AlphaOx, 1))^(1/Num_timesteps)) * Ox
        res_vec$Offtake_Ox[month] <- (1-(1-sample(GammaOx, 1))^(1/Num_timesteps)) * Ox
      ...
      if (species == "cattle" || species == "smallruminants") {
        ...
        res_vec$Value_milk[month] <- Quantity_milk[month] * sample(milk_value_ltr, 1)
    ...
    if (species == "cattle" || species == "smallruminants" || species == "poultry") {
        ...
        if (species == "cattle" || species == "smallruminants" || species == "poultry") {
          ...
          res_vec$Labour_cost_AF[month] <- Labour_AF + AF * (sample(Labour_cost_head_AF, 1)/Num_timesteps) * lab_non_health + AF * prop_F_milked * (sample(Labour_cost_head_dairy, 1)/Num_timesteps)  
      ...
      if (species == "cattle") {
        res_vec$Health_cost_Ox[month] <- Health_Ox + Ox * (sample(Health_exp_prev, 1)/Num_timesteps) + Ox * (sample(Health_exp_treatment, 1)/Num_timesteps) 
...
#    res_mat$Monthly_mortality[i, ] <- res_vec$Monthly_mortality # remove
...
#    res_mat$Monthly_growth_rate[i, ] <- res_vec$Monthly_growth_rate  # remove
#    res_mat$Monthly_pop_growth[i, ] <- res_vec$Monthly_pop_growth    # remove
    ...
    ## MILK VALUEADDED
    res$mat$Value_milk[i, ] <- res_vec$Value_milk 
  ...
  #  res_mat$Monthly_DM[i, ] <- res_vec$Monthly_DM # remove
 ...
 #     res_mat$Total_mortality_Ox[i, ] <- res_vec$Total_mortality_Ox
  ...
  if (species == "cattle" || species == "smallruminants") {
    ...
    ## REMOVE THIS Section as filled in value matrix above in simulation  
  ...
  #                     Value_hides_AM)
    ...
    res_mat$Production_value_herd_offtake_hide_manure <- sum(res_mat$Production_value_herd_offtake_hide_manure_JF,
  ...
  if (species == "cattle") {
    ...
    Gross_margin_Ox <- res_mat$Production_value_herd_offtake_hide_manure_Ox - res_mat$Total_expenditure_Ox
  }
