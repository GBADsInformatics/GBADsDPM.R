...
run_compartmental_model <- function(seed_value = NULL) {
  ...
  for (i in 1:nruns) {
    ...
    for (month in 1:Num_months) {
      ...
      res_vec$Value_of_Total_Mortality_NM[month] <- res_vec$Total_Mortality_NM[month] * sample(fvNM, 1)
      ...
      if (species == "cattle") {
        res_vec$Value_of_Total_Mortality_O[month] <- res_vec$Total_Mortality_O[month] * sample(fvO, 1)
